...
class DecisionMatrix {
    ...
    renderProgressIndicator() {
        ...
        html += `<div class="progress-bar" data-progress="${progressPercentage}"></div>`;
        html += '</div>';
        ...
        steps.forEach((step, index) => {
            ...
            const icon = step.complete ? '✓' : (index === completedSteps ? '→' : (index + 1));
            html += `
        ...
        // Set progress bar width using JavaScript (CSP-compliant)
    ...
    calculateResults() {
        ...
        this.showToast('Results calculated successfully! 🎉');
    }
